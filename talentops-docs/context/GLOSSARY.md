...
| **Hybrid Loop** | Two-phase architecture (D19) for bias-safe evaluation: Phase 1 (live) — `gemini-3.1-flash-live-preview` conducts the audio conversation and auto-generates a raw text transcript; Phase 2 (async/offline) — that raw text transcript is fed to the Analytics/Scorecard sub-agent (Groq Llama 3.3 70B / OpenRouter Nemotron) which performs ALL evaluation exclusively on text. The text transcript is the structural blind wall between voice and scorer (same enforcement class as D17’s STT boundary). |
